    ...
    switch( message )
    {
		...
		case WM_ACTIVATEAPP:

			{
				...
				bool bHasFocus = (((int)wParam != 0));
				CCoreEngine::Get()->SetFocus(bHasFocus);
			}
...
void Update()
{
	if (!CCoreEngine::Get()->HasFocus())
		return;

	// check if we have any file change events to process
	CCoreEngine::Get()->GetFileSystem()->CheckModifiedFiles();
